...
import com.hugo83.backboard.dto.Header;
import com.hugo83.backboard.dto.PagingDto;
...
public class RestBoardController {
    ...
    @ResponseBody
    public Header<List<BoardDto>> list(Model model,
...
//         List<Board> list = pages.getContent();
       ...
       for (Board origin : pages){
           ...
           bdDto.setWriter(origin.getWriter() != null ? origin.getWriter().getUsername() : ""); // null 에
...
//      Board board = this.boardService.getBoard(bno);
        ...
        if (_board.getReplyList().size() > 0) {
            _board.getReplyList().forEach(rpy -> replyList.add(ReplyDto.builder().content(rpy.getContent())
                                                            ...
                                                            .rno(rpy.getRno()).writer(rpy.getWriter().getUsername())
